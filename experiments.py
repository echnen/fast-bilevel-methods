...
def experiment_nemirovisky():
    ...
    for cs in tqdm(range(cases)):
        ...
        Res_Bi_PG[:, cs], Obj_Bi_PG[:, cs], Obj_H_Bi_PG[:, cs] = opt.Bi_PG(x_init, sigma_e, s, c, delta, Model, maxit)

        # Algorithm 2 (our paper)
        Res_biFI[:, cs], Obj_biFI[:, cs], Obj_H_biFI[:, cs] = opt.bi_FISTA(x_init, alpha, sigma_e, sigma_t, s, c, delta, Model, maxit)

        # Fast Bi-level Proximal Gradient (Merchav, Sabach, Teboulle, '24)
        Res_FBi_PG[:, cs], Obj_FBi_PG[:, cs], Obj_H_FBi_PG[:, cs] = opt.FBi_PG(x_init, alpha, s, c, delta, Model, maxit)

        # Static Bilevel Method (Latafat, Themelis, Villa, Patrinos, '24)
        Res_staBiM[:, cs], Obj_staBiM[:, cs], Obj_H_staBiM[:, cs] = opt.staBiM(x_init, sigma_e, c, delta, Model, maxit)

        # Bi-Sub-Gradient - Version II (Merchav, Sabach, '23)
        Res_Bi_SG_II[:, cs], Obj_Bi_SG_II[:, cs], Obj_H_Bi_SG_II[:, cs] = opt.Bi_SG_II(x_init, c, delta, Model, maxit)


    show.plot_nemirovsky(Res_Bi_PG, Res_biFI, Res_FBi_PG, Res_staBiM, Res_Bi_SG_II,
...
def experiment_logistic():
    ...
    Res_Bi_PG, Obj_Bi_PG, Obj_H_Bi_PG = opt.Bi_PG(x_init, sigma_e, s, c, delta, Model, maxit)

    # Algorithm 2 (our paper)
    print('Starting Algorithm 2 ...')
    Res_biFI, Obj_biFI, Obj_H_biFI = opt.bi_FISTA(x_init, alpha, sigma_e, sigma_t,
                                                  s, c, delta, Model,
                                                  maxit)

    # Fast Bi-level Proximal Gradient (Merchav, Sabach, Teboulle, '24)
    print('Starting Fast Bi-Level Proximal Gradient ...')
    Res_FBi_PG, Obj_FBi_PG, Obj_H_FBi_PG = opt.FBi_PG(x_init, alpha, s, c,
                                                      delta, Model, maxit)

    # Static Bilevel Method (Latafat, Themelis, Villa, Patrinos, '24)
    print('Starting Static Bilevel Method ...')
    Res_staBiM, Obj_staBiM, Obj_H_staBiM = opt.staBiM(x_init, sigma_e, c,
                                                      delta, Model, maxit)

    # Bi-Sub-Gradient - Version II (Merchav, Sabach, '23)
    print('Starting Bi-Sub-Gradient Version II ...')
    Res_Bi_SG_II, Obj_Bi_SG_II, Obj_H_Bi_SG_II= opt.Bi_SG_II(x_init, c,
                                                             delta, Model,
                                                             maxit)
